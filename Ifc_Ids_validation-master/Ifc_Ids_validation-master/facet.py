...
class Entity(Facet):
    def __init__(self, name="IFCWALL", predefinedType=None, instructions=None):
        ...
        self.applicability_templates = [
            "Проверяются элементы с классом {name} и типом {predefinedType}", #All {name} data of type {predefinedType}
            "Проверяются элементы с классом {name}", #All {name} data
        ]
        self.requirement_templates = [
            "Элементы должны иметь класс {name} с типом {predefinedType}", #Shall be {name} data of type {predefinedType}
            "Элементы должны иметь класс {name}", #Shall be {name} data
        ]
        self.prohibited_templates = [
            "Класс элементов не должен быть {name} с типом {predefinedType}", #Shall not be {name} data of type {predefinedType}
            "Класс элементов не должен быть {name}", #Shall not be {name} data
        ]
...
class Attribute(Facet):
    def __init__(self, name="Name", value=None, minOccurs=None, maxOccurs=None, instructions=None):
        ...
        self.applicability_templates = [
            "Проверяются элементы с атрибутом {name} имеющим значение {value}", #Data where the {name} is {value}
            "Проверяются элементы с атрибутом {name}", #Data where the {name} is provided
        ]
        self.requirement_templates = [
            "Атрибут {name} должен иметь значение {value}", #The {name} shall be {value}
            "Должен быть атрибут {name}", #The {name} shall be provided
        ]
        self.prohibited_templates = [
            "Атрибута {name} не должен иметь значение {value}", #The {name} shall not be {value}
            "Не должно быть атрибута {name}", #The {name} shall not be provided
        ]
...
class Classification(Facet):
    def __init__(self, value=None, system=None, uri=None, minOccurs=None, maxOccurs="unbounded", instructions=None):
        ...
        self.applicability_templates = [
            "Проверяются элементы с кодом '{value}' из системы классификации '{system}'", #Data having a {system} reference of {value}
            "Проверяются элементы закодированные по системе классификации '{system}'", #Data classified using {system}
            "Проверяются элементы с кодом '{value}'", #Data classified as {value}
            "Проверяются элементы имеющие классификацию", #Classified data
        ]
        self.requirement_templates = [
            "Элементы должны иметь код '{value}' из системы классификации '{system}'", #Shall have a {system} reference of {value}
            "Элементы должны быть закодированны по системе классификации '{system}'", #Shall be classified using {system}
            "Элементы должны иметь код '{value}'", #Shall be classified as {value}
            "Элементы должны иметь классификацию", #Shall be classified
        ]
        self.prohibited_templates = [
            "Элементы не должны иметь код '{value}' из системы классификации '{system}'", #Shall not have a {system} reference of {value}
            "Элементы не должны быть закодированны по системе классификации '{system}'", #Shall not be classified using {system}
            "Элементы не должны иметь код '{value}'", #Shall not be classified as {value}
            "Элементы должны иметь классификацию", #Shall not be classified
        ]
...
class PartOf(Facet):
    def __init__(
        ...
        self.applicability_templates = [
            "Проверяются элементы имеющие связь '{relation}' с классом '{name}'", #An element with an {relation} relationship with an {name}
            "Проверяются элементы имеющие связь '{relation}'", #An element with an {relation} relationship
        ]
        self.requirement_templates = [
            "Элемент должен иметь связь '{relation}' с классом '{name}'", #An element must have an {relation} relationship with an {name}
            "Элемент должен иметь связь '{relation}'", #An element must have an {relation} relationship
        ]
        self.prohibited_templates = [
            "Элемент не должен иметь связь '{relation}' с классом '{name}'", #An element must not have an {relation} relationship with an {name}
            "Элемент не должен иметь связь '{relation}'", #An element must not have an {relation} relationship
        ]
...
class Property(Facet):
    def __init__(
        ...
        self.applicability_templates = [
            "Проверяются элементы имеющие в наборе параметров '{propertySet}' параметр '{name}' со значением '{value}' ", #Elements with {name} data of {value} in the dataset {propertySet}
            "Проверяются элементы имеющие в наборе параметров '{propertySet}' параметр '{name}' ", #Elements with {name} data in the dataset {propertySet}
        ]
        self.requirement_templates = [
            "В наборе параметров '{propertySet}' должен быть параметр '{name}' со значением {value}", #{name} data shall be {value} and in the dataset {propertySet}
            "В наборе параметров '{propertySet}' должен быть параметр '{name}'", #{name}data shall be provided in the dataset {propertySet}
        ]
        self.prohibited_templates = [
            "В наборе параметров '{propertySet}' не должно быть параметра '{name}' со значением '{value}' ", #{name} data shall not be {value} and in the dataset {propertySet}
            "В наборе параметров '{propertySet}' не должно быть параметра '{name}'", #{name} data shall not be provided in the dataset {propertySet}
        ]
...
class Material(Facet):
    def __init__(self, value=None, uri=None, minOccurs=None, maxOccurs="unbounded", instructions=None):
        ...
        self.applicability_templates = [
            "Проверяются элементы с назначенным материалом '{value}'",     # All data with a {value} material
            "Проверяются элементы с назначенным материалом",     # All data with a material
        ]
        self.requirement_templates = [
            "Элементу должен быть назначен материал '{value}'",     # Shall have a material of {value}
            "Элементу должен быть назначен материал",        # Shall have a material
        ]
        self.prohibited_templates = [
            "Элементу не должен быть назначен материал '{value}'",     # Shall not have a material of {value}
            "Элементу не должен быть назначен материал",        # Shall not have a material
        ]
...
class Result:
    ...
    def to_string(self):
        return str(self.reason) or "Неизвестная ошибка при проверке выполнения требований"     # The requirements were not met for some inexplicable reason. Good luck!


class EntityResult(Result):
    def to_string(self):
        if self.reason["type"] == "NAME":
            return f"Класс элемента \"{self.reason['actual']}\" не соответствует требуемому"     # The entity class \"{self.reason['actual']}\" does not meet the required IFC class
        elif self.reason["type"] == "PREDEFINEDTYPE":
            return f"Тип элемента \"{str(self.reason['actual'])}\" не соответствует требуемому"      # The predefined type \"{str(self.reason['actual'])}\" does not meet the required type


class AttributeResult(Result):
    def to_string(self):
        if self.reason["type"] == "NOVALUE":
            return "Требуемый атрибут не существует"     # The required attribute did not exist
        elif self.reason["type"] == "FALSEY":
            return f"Значение требуемого атрибута \"{str(self.reason['actual'])}\" пустое"     # The attribute value \"{str(self.reason['actual'])}\" is empty
        elif self.reason["type"] == "INVALID":
            return f"Указано не допустимое имя атрибута"     # An invalid attribute name was specified in the IDS
        elif self.reason["type"] == "VALUE":
            return f"Значение атрибута \"{str(self.reason['actual'])}\" не соответствует требованию"     # The attribute value \"{str(self.reason['actual'])}\" does not match the requirement
        elif self.reason["type"] == "PROHIBITED":
            return f"Введено недопустимое значение"     # The attribute value should not have met the requirement


class ClassificationResult(Result):
    def to_string(self):
        if self.reason["type"] == "NOVALUE":
            return "Элемент не имеет классификации"       # The entity has no classification
        elif self.reason["type"] == "VALUE":
            return f"Код элемента \"{str(self.reason['actual'])}\" не соответствует требованию"     # The references \"{str(self.reason['actual'])}\" do not match the requirements
        elif self.reason["type"] == "SYSTEM":
            return f"Система классификации \"{str(self.reason['actual'])}\" не соответствует требованию"        # The systems \"{str(self.reason['actual'])}\" do not match the requirements
        elif self.reason["type"] == "PROHIBITED":
            return f"Введено недопустимое значение для классификации"        # The classification should not have met the requirement


class PartOfResult(Result):
    def to_string(self):
        if self.reason["type"] == "NOVALUE":
            return "Элемент не имеет связей"     # The entity has no relationship
        elif self.reason["type"] == "ENTITY":
            return f"Элемент имеет связи с не корректными сущностями: \"{str(self.reason['actual'])}\""       #TODO # The entity has a relationship with incorrect entities: \"{str(self.reason['actual'])}\"
        elif self.reason["type"] == "PROHIBITED":
            return f"Имеется недопустимое значение связей"      # The relationship should not have met the requirement


class PropertyResult(Result):
    def to_string(self):
        if self.reason["type"] == "NOPSET":
            return "Требуемый набор параметров не существует"       # The required property set does not exist
        elif self.reason["type"] == "NOVALUE":
            return "Набор параметров не содержит требуемого параметра"  # The property set does not contain the required property
        elif self.reason["type"] == "DATATYPE":
            return f"Тип данных \"{str(self.reason['actual'])}\" не соответствует требуемому типу данных \"{str(self.reason['datatype'])}\""     # The property's data type \"{str(self.reason['actual'])}\" does not match the required data type of \"{str(self.reason['datatype'])}\"
        elif self.reason["type"] == "VALUE":
            if isinstance(self.reason["actual"], list):
                if len(self.reason["actual"]) == 1:
                    return f"Значение параметра \"{str(self.reason['actual'][0])}\" не соответствует требованиям"        # The property value \"{str(self.reason['actual'][0])}\" does not match the requirements
                else:
                    return f"Значение параметров \"{str(self.reason['actual'])}\" не соответствует требованиям"        # The property values \"{str(self.reason['actual'])}\" do not match the requirements
            else:
                return f"Значение параметра \"{str(self.reason['actual'])}\" не соответствует требованиям"       # The property value \"{str(self.reason['actual'])}\" does not match the requirements
        elif self.reason["type"] == "PROHIBITED":
            return f"Значение параметра имеет недопцустимое значение"      # The property should not have met the requirement


class MaterialResult(Result):
    def to_string(self):
        if self.reason["type"] == "NOVALUE":
            return "Элементу не назначен материал"     # "The entity has no material"
        elif self.reason["type"] == "VALUE":
            return (
                f"Наименование материала \"{str(self.reason['actual'])}\" не соответствует требованиям"     # The material names and categories of \"{str(self.reason['actual'])}\" does not match the requirement
            )
        elif self.reason["type"] == "PROHIBITED":
            return f"Заданный материал не допускается"      # The material should not have met the requirement
